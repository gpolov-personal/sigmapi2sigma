...
import { formatDuration, sessionTitle } from "../utils";
                ...
                <div key={c.sessionId} className="border border-slate-800 rounded p-3 bg-slate-950/40">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      {(() => {
                        const title = sessionTitle(c);
                        if (!title) return null;
                        return (
                          <div
                            className={`text-sm truncate ${title.source === "custom" ? "text-slate-200" : "text-slate-400 italic"}`}
                            title={title.source === "custom" ? "Set with /rename" : "Auto-generated by Claude Code"}
                          >{title.text}</div>
                        );
                      })()}
                      <div className="font-mono text-xs text-slate-500">{c.sessionId.slice(0, 12)}…</div>
                    </div>
