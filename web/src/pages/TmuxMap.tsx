...
import { copy, relativeTime, trunc, sessionTitle } from "../utils";
      ...
      {isClaude && (pane.claudeSessionId || pane.claudeAccount) && (
        <div className="mb-2 text-xs space-y-0.5">
          {pane.claudeSessionId ? (
            <>
              <div className="text-cyan-400/70">Session:</div>
              ...
              <PaneConversationName pane={pane} />
            </>
          ) : (
            pane.cmd === "claude" && pane.claudeAccount && <AccountBadge accounts={[pane.claudeAccount]} />
          )}
...
// The conversation running in this pane, by name. A /rename reads as-is; Claude
// Code's own auto title is dimmed and italic so the two stay distinguishable.
// Snapshot-restored panes predating name tracking simply render nothing.
function PaneConversationName({ pane }: { pane: TmuxPane }) {
  if (!pane.claudeSessionId) return null;
  const title = sessionTitle({
    customTitle: pane.claudeCustomTitle ?? null,
    aiTitle: pane.claudeAiTitle ?? null,
  });
  return (
    <div className="pl-3 flex gap-2 min-w-0">
      <span className="text-slate-500 shrink-0">name:</span>
      {title ? (
        <span
          className={`truncate ${title.source === "custom" ? "text-cyan-100" : "text-cyan-200/60 italic"}`}
