...
4. **Multiple distinct conversations per LWD are supported** (superseded — this invariant used to say the opposite). Running a checker and an implementer from the same launch directory is a normal workflow. `claude --resume <uuid>` takes an explicit UUID and is only directory-scoped for the *lookup*, so two conversations in one directory resume fine — the old "one conversation per LWD" rule was never a Claude Code constraint, it was a rule invented to keep the mtime resolver's guess correct.

   Pane→conversation identity now comes from the `SessionStart` hook (`sp2s-bind.sh`, installed from the dotfiles repo), which writes `~/.sigmapi2sigma/pane-bindings.jsonl` keyed on tmux `paneId`. It re-fires on `/clear`, `resume`, `compact` and `fork`, so a binding follows the pane instead of freezing at launch. Resolvers prefer the binding and fall back to newest-mtime, reporting `claudeSessionSource: "binding" | "mtime"` so a guess is never presented as fact. **The fallback returns the same id for every pane in a directory** — that is the failure mode the hook exists to remove, so never treat mtime-derived ids as distinguishing panes.

   What *is* flagged now is the inverse: **one conversation open in more than one pane** (two processes appending to one transcript). The Tmux Map warns on that, counting only `binding`-sourced ids — counting mtime guesses would report a duplicate for every multi-pane project, since they are identical by construction.
