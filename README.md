...
| 🧠 **AI conversation tracking** | When a pane is running an AI CLI that writes session logs (current implementation: [Claude Code](https://github.com/anthropics/claude-code)), the relevant logs are surfaced inline — you see *which conversations* you had during *which pomodoro*. Conversations are listed by **name**: whatever you set with Claude Code's `/rename`, falling back to its own auto-generated topic title. Rename again and the new name follows on the next refresh. |
